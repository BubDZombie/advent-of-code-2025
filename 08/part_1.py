...
if __name__ == "__main__":
    ...
    for n1, n2, distance in sorted(distances, key=lambda d: d[2])[:linked_target]:
    ...
    product = 1
    for circuit in sorted(circuits, key=lambda c: len(c.nodes), reverse=True)[:3]:
        product *= len(circuit.nodes)
    print(product)
